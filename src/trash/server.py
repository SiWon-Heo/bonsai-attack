...
class Server:
    def __init__(self, make_middle, clients, trainset, batch_size):
        self.server = make_middle()
        self.clients = clients
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=LR)

        self.trainset = (
            trainset.shuffle(BUFFER_SIZE)
            .batch(batch_size, drop_remainder=True)
            .repeat(-1)
        )

    def train_step(
        self, step0, step1, client_trainable_variables, x=None, y=None
    ):
        ...
        for i in tqdm.trange(iterations):
            ...
            for j, client in enumerate(self.clients):
                ...
                if client.isbad:
                    if i % 2 == 0:
                        # train generator
                        log_bad = self.train_step(
                            client.step0_G,
                            client.step1_G,
                            client.trainable_variables_G(),
                        )
